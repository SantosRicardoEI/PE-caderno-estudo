...
### Exemplo 0 — $E(X)$ e $V(X)$ por integração (densidade dada por ramos)
Procura diária de arroz (em centenas de kg), com densidade:
$$f_X(x)=\begin{cases}\tfrac{2x}{3}, & 0\le x<1\\[2pt] -\tfrac{x}{3}+1, & 1\le x\le 3\\[2pt] 0, & \text{c.c.}\end{cases}$$

**Passo 1 — valor esperado** $E(X)=\int x\,f_X(x)\,dx$, separando pelos ramos:
$$E(X)=\int_0^1 x\cdot\tfrac{2x}{3}\,dx+\int_1^3 x\left(-\tfrac{x}{3}+1\right)dx=\left[\tfrac{2x^3}{9}\right]_0^1+\left[-\tfrac{x^3}{9}+\tfrac{x^2}{2}\right]_1^3=\tfrac{4}{3}$$

**Passo 2 — $E(X^2)=\int x^2 f_X(x)\,dx$** (precisas dele para a variância):
$$E(X^2)=\int_0^1 x^2\cdot\tfrac{2x}{3}\,dx+\int_1^3 x^2\left(-\tfrac{x}{3}+1\right)dx=\left[\tfrac{2x^4}{12}\right]_0^1+\left[-\tfrac{x^4}{12}+\tfrac{x^3}{3}\right]_1^3=\tfrac{13}{6}$$

**Passo 3 — variância** (fórmula prática):
$$V(X)=E(X^2)-[E(X)]^2=\tfrac{13}{6}-\left(\tfrac{4}{3}\right)^2=\tfrac{7}{18}$$

> **Método:** parte o integral pelos ramos da densidade; $E(X)$ usa $x\,f_X$, $E(X^2)$ usa $x^2 f_X$; a variância sai de $E(X^2)-E^2(X)$. A **mediana/quantis** obtêm-se resolvendo $F_X(\mu_e)=0{,}5$ (integra a densidade até ao ponto).
